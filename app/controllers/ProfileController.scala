...
Console.println(maybeUser)
      maybeUser match {
        case Some(result) if (user.equals(result.username) && result.isAdmin==false)=>Ok(views.html.profile("",messages,result.firstname,result.middlename,result.lastname,user,result.mobile,result.gender,result.age,result.hobbies))
        case Some(result) if (user.equals(result.username) && result.isAdmin==true)=>Ok(views.html.profile("Maintenance",messages,result.firstname,result.middlename,result.lastname,user,result.mobile,result.gender,result.age,result.hobbies))
